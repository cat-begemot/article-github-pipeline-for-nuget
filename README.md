...
В этой статье на практическом примере я хочу показать как настроить CI/DI [GitHub actions](https://github.com/features/actions) для валидации и развертывания NuGet пакета, начиная с минимально полезного конфигурационного `yaml` файла и постепенного добавления функционала до полной автоматизации запланированных требований.
...
### Необходмые этапы для публикации в ручном режиме
...
После внесения изменений в нашу библиотеку необходимо раз за разом выполнять перечисленные выше 6 шагов для доставки новой версии пакета конечным пользователем. Возьмем эти пункты в качестве требований и автоматизируем процесс.
